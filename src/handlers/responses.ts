...
export class ResponseHandler {
  ...
  async handleResponsesWithBody(requestBody: ResponseRequest, apiKey: string): Promise<Response> {
    try {
      // Validate required fields - support both input and messages formats
      if (!requestBody.input && (!requestBody.messages || !Array.isArray(requestBody.messages))) {
        return createErrorResponse('Either "input" field (string) or "messages" field (array) is required');
      }
  ...
  private async handleNonStreamingResponse(
    ...
    try {
      ...
      const openAIResponse = this.transformToResponsesFormat(data, model, responseFormat);
  ...
  private enhanceMessagesForStructuredResponse(
    ...
    if (responseFormat) {
      ...
      switch (responseFormat.type) {
        case 'json_object':
          structurePrompt = 'Please respond with a valid JSON object only. Do not include any text outside the JSON structure.';
          break;
        case 'json_schema':
        ...
        case 'text':
        default:
          structurePrompt = 'Please provide a clear and structured text response.';
          break;
      }
      ...
      if (reasoningEffort) {
        const effortInstructions = {
          ...
          medium: 'Think through the problem step by step and provide a well-reasoned response.',
  ...
  private transformToResponsesFormat(data: any, model: string, responseFormat?: ResponseRequest['response_format']): any {
    const content = data.aiRecord?.aiRecordDetail?.resultObject?.[0] || data.content || 'No response generated';
